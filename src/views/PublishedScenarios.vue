<template>
  <div class="catalog-page">
    <header class="page-header">
      <div><h1>已发布场景</h1><p>Published Scenario Catalog · 每个发布版本都是不可变业务快照，可被委托 / 申请在运行时引用。</p></div>
      <div class="header-actions"><el-button size="small" @click="router.push('/app/scenarios/activation')">场景激活</el-button><el-button type="primary" size="small" @click="router.push('/app/scenarios')">场景包管理</el-button></div>
    </header>
    <div class="governance-notice"><el-icon><Lock /></el-icon><div><strong>Published Version 不可直接编辑。</strong><span>新委托绑定当时的 Scenario Snapshot；已有运行实例不会被后续版本升级或场景停用静默改变。</span></div></div>
    <div class="catalog-panel">
      ...
      <el-table :data="filteredScenarios" border size="small" @row-click="openScenario" class="catalog-table">
        <el-table-column label="场景" min-width="210" fixed><template #default="{ row }"><div class="scene-cell"><strong>{{ row.name }}</strong><span>{{ row.key }}</span></div></template></el-table-column>
    ...
    <el-drawer v-model="drawerOpen" size="640px" :with-header="false" aria-label="场景详情">
      <div v-if="selected" class="drawer-shell">
        <header class="drawer-header">
          <div><div class="drawer-title-row"><h2>{{ selected.name }}</h2><span class="reference-token">{{ selected.version }}</span><span class="immutable-token"><el-icon><Lock /></el-icon>Immutable</span></div><div class="drawer-key">{{ selected.key }}</div></div>
          <el-button type="primary" size="small" @click="newVersion(selected)"><el-icon><Plus /></el-icon>创建新版本</el-button>
          <el-button class="drawer-close" text circle aria-label="关闭场景详情" @click="drawerOpen = false"><el-icon><Close /></el-icon></el-button>
        </header>
        <nav class="drawer-tabs"><button v-for="tab in tabs" :key="tab.key" :class="{ active: activeTab === tab.key }" @click="activeTab = tab.key">{{ tab.label }}</button></nav>
        <section v-if="activeTab === 'overview'" class="drawer-content">
          ...
          <div class="detail-grid"><div><span>业务模式</span><strong>{{ selected.mode }}</strong></div><div><span>领域</span><strong>{{ selected.domain }}</strong></div><div><span>Workflow</span><strong>{{ selected.workflowVersion }}</strong></div><div><span>检测项</span><strong>{{ selected.itemCount }} 项</strong></div><div><span>标准 / 方法</span><strong>{{ selected.standardCount }} / {{ selected.methodCount }}</strong></div><div><span>报告模板</span><strong>{{ selected.reportVersion }}</strong></div></div>
        ...
        <section v-else-if="activeTab === 'versions'" class="drawer-content">
          ...
          <div class="version-list"><div v-for="version in selected.versions" :key="version.version" class="version-row"><div class="version-line"><span class="reference-token">{{ version.version }}</span><span :class="['version-status', version.status.toLowerCase()]">{{ version.status }}</span></div><div class="version-copy"><strong>{{ version.note }}</strong><span>{{ version.time }}</span><span v-if="version.hash" class="hash-text">sha256:{{ version.hash }}</span></div></div></div>
...
const baseDependencies = [{ type: 'STANDARD', ref: 'GB 5009.12-2023' }, { type: 'METHOD', ref: 'ICP-MS Pb v2.1' }, { type: 'REPORT', ref: 'food-physchem-report v4.2' }, { type: 'AI_SKILL', ref: 'report-draft v2.0' }]
const scenarios = ref([
  {
...
@media(max-width:767px){.drawer-header{padding-right:52px}}
</style>
