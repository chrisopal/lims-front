<template>
  <div class="app-layout">
    <header class="topbar">
      ...
      <div class="topbar-right">
        <el-tooltip content="消息" placement="bottom">
          <el-badge :value="pending" :hidden="!pending" class="topbar-icon"
            ><el-button
              text
              circle
              aria-label="消息"
              @click="router.push('/app/operations/my-work')"
              ><el-icon size="17"><Bell /></el-icon></el-button
          ></el-badge>
        </el-tooltip>
    ...
    <div class="app-body">
      ...
      >
        ...
        >
          <el-menu-item index="/app/dashboard"
            ><el-icon><Grid /></el-icon><span>工作台</span></el-menu-item
          >
      ...
      <main class="main-content" :inert="isMobile && mobileOpen">
        <router-view />
      </main>
    </div>
  </div>
...
const pending = computed(
  () =>
    runtime.state.value.workItems.filter((w) =>
      ['READY', 'IN_PROGRESS'].includes(w.status),
    ).length,
)
...
const activeMenu = computed(() =>
  route.path === '/app/scenarios/studio' ? '/app/scenarios' : route.path,
)
const menuGroups = [
  {
    label: '业务运行',
...
function trapNavigationFocus(event: KeyboardEvent) {
  ...
  const items = Array.from(
    navigation.value?.querySelectorAll<HTMLElement>('.el-menu-item') ?? [],
  )
