...
const statuses: Record<string, { label: string; tone: string }> = {
  ...
  READY: { label: '待处理', tone: 'neutral' },
  IN_PROGRESS: { label: '进行中', tone: 'info' },
  COMPLETED: { label: '已完成', tone: 'success' },
  RETURNED: { label: '已退回', tone: 'warning' },
}
const value = computed(
  () => statuses[props.status] || { label: props.status, tone: 'neutral' },
)
</script>
<template>
  <span class="runtime-status" :class="value.tone"
    ><span aria-hidden="true">●</span>{{ value.label }}</span
  >
</template>
<style scoped>
.runtime-status {
